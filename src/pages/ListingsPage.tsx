...
export function ListingsPage() {
  ...
  return <>
    <Seo title="Property Listings" description="Browse current homes, apartment investments and land opportunities marketed by Mafick Integrated across Lagos."/>
    <section className="bg-[#17352C] px-5 py-16 text-white sm:px-8 lg:py-24"><div className="mx-auto max-w-[1200px]"><p className="mb-4 font-bold text-[#D3AC67]">Current property opportunities</p><h1 className="font-display text-5xl sm:text-7xl">Homes, land and investment property in Lagos.</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-white/70">Explore the property information currently available to our team, then contact us to confirm availability, receive current media and arrange an inspection.</p></div></section>
    <section className="sticky top-[76px] z-30 border-b border-[#C5CEC7] bg-[#F7F8F5]/95 px-4 py-4 backdrop-blur"><div className="mx-auto flex max-w-[1350px] flex-wrap gap-3"><label className="text-xs font-bold text-[#59635E]">Property type<select className="ml-2 min-h-11 border border-[#AEB8B1] bg-white px-3 text-sm" value={propertyType} onChange={e=>setPropertyType(e.target.value)}><option value="all">All types</option><option value="land">Land</option><option value="house">Houses</option><option value="apartment">Apartments</option><option value="commercial">Investment / commercial</option></select></label><label className="text-xs font-bold text-[#59635E]">Location<select className="ml-2 min-h-11 border border-[#AEB8B1] bg-white px-3 text-sm" value={location} onChange={e=>setLocation(e.target.value)}><option value="all">All locations</option>{locations.map(item=><option key={item} value={item}>{item}</option>)}</select></label><label className="text-xs font-bold text-[#59635E]">Availability<select className="ml-2 min-h-11 border border-[#AEB8B1] bg-white px-3 text-sm" value={status} onChange={e=>setStatus(e.target.value)}><option value="all">All availability</option><option value="For sale">For sale</option><option value="For rent">For rent</option></select></label></div></section>
